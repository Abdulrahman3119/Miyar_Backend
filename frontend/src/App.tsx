...
import { Bell, Globe, ShieldCheck, BookOpen, MessageSquare, Phone } from 'lucide-react'
...
function Help() {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4"><Kpi label="تذاكري المفتوحة" value={openTickets.length} hint="من الخادم" /><Kpi label="أدلة الاستخدام" value={help.guides.length} icon={BookOpen} /><Kpi label="الدعم الهاتفي" value={<span className="ltr text-[20px]">{help.support.phone || '—'}</span>} icon={Phone} hint={help.support.hours} /></div>
      <div className="mt-3 grid grid-cols-12 gap-3">
        <div className="col-span-12 grid content-start gap-3 lg:col-span-5"><Section title="أدلة سريعة" icon={BookOpen} bodyClass="p-0"><ul className="divide-y divide-ink-100">{help.guides.map(g => <li key={g.id} className="flex items-center gap-3 px-3 py-2 text-[12.5px]"><span className="min-w-0 flex-1"><span className="block truncate font-medium">{g.title}</span><span className="meta">{g.audience} · {g.minutes} دقائق</span></span><button type="button" className="shrink-0 text-[11.5px] font-semibold text-brand-700 hover:underline" onClick={() => useStore.getState().toast({ title: g.title, body: g.body.slice(0, 280), tone: 'info' })}>فتح</button></li>)}{!help.guides.length && <li className="px-3 py-3 text-[12px] text-ink-500">لا أدلة بعد.</li>}</ul></Section><Section title="التواصل" icon={Phone} bodyClass="p-3"><KV cols={1} dense items={[{ k: 'الهاتف', v: <span className="ltr">{help.support.phone || '—'}</span> }, { k: 'البريد', v: help.support.email || '—' }, { k: 'الطوارئ الميدانية', v: <span className="ltr">{help.support.emergency || '—'}</span> }]} /></Section></div>
        ...
        <div className="col-span-12 grid content-start gap-3 lg:col-span-3"><Section title="تذاكري" icon={MessageSquare} bodyClass="p-0"><ul className="divide-y divide-ink-100">{tickets.map(t => <li key={t.id} className="px-3 py-2 text-[12px]"><div className="flex items-center justify-between"><span className="font-semibold text-brand-700">{t.id}</span><Badge tone={ticketTone(t.status) as any} size="xs">{t.status}</Badge></div><div className="truncate text-ink-700">{t.subject}</div><div className="meta">{t.at}</div></li>)}{!tickets.length && <li className="px-3 py-3 text-[12px] text-ink-500">لا تذاكر بعد.</li>}</ul></Section>
          <Section title="روابط مفيدة" bodyClass="p-3"><ul className="grid gap-1.5 text-[12px]">{help.links.map(l => <li key={l.url} className="flex items-center justify-between gap-2"><span className="truncate">{l.title}</span><span className="ltr meta shrink-0">{l.url.replace(/^https?:\/\//, '')}</span></li>)}</ul></Section>
        </div>
      </div>
